...
export class CritereEvaluationService {
  private apiUrl = 'http://192.168.79.129:9097/api/criteres'; // URL centrale pour toutes les requêtes

  constructor(private http: HttpClient) {}

  getAll(): Observable<CritereEvaluationDTO[]> {
    return this.http.get<CritereEvaluationDTO[]>('http://192.168.79.129:9097/api/criteres');
  }
  ...
  deleteCriteres(ids: string[]) {
    ...
    return this.http.delete(`http://192.168.79.129:9097/api/criteres`, { 
      body: { ids }  // Passer les IDs des critères à supprimer
    });
  }

  private apiUrlcritere = 'http://192.168.79.129:9097/api/criteres';
...
getCriteresBySprintId(sprintId: string): Observable<CritereEvaluationDTO[]> {
  return this.http.get<CritereEvaluationDTO[]>(`http://192.168.79.129:9097/api/criteres/sprints/${sprintId}`);
}
